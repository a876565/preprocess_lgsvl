...
def normalize(pcl,visutalization=False):
    ...
    #pcd = down_sample(pcd)
    #noised_pcl=add_noise_points3(np.asarray(pcd.points),-1,10,[0.05,0.05,0.03],(1.20,1.20,0.15),(-0.60,-0.60,0.05),visutalization)
    #pcd.points=o3d.utility.Vector3dVector(noised_pcl)
    pcd = down_sample(pcd)
    #print(' downsample:',np.asarray(pcd.points).shape)
    ...
    return pcl-new_pos,new_pos*np.array([60,60,30])
...
def display_pcl(cloud,name='Open3D Removal Outlier'):
    #     input [N, 3]
    pcd = o3d.geometry.PointCloud()
