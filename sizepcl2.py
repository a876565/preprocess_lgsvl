...
def read_pcl(dir,record):
    print('read ',record[0])
    fname=os.path.join(dir,'pointcloud_20m_10overlap',record[0]+'.bin')
    pcl=np.fromfile(fname,dtype=np.float64).reshape(-1, 3)
    print_pcl_info(pcl)
    if len(record)>=3:
        pos=np.array(record[1:4]).astype(np.float32)
        pcl=pcl + pos*np.array([1/100,1/100,1/30])
    return pcl

dir1='/media/mustar/DATA4/LGSVL/benchmark_datasets/kitti/05'
dir2='/media/mustar/DATA4/LGSVL/benchmark_datasets/kitti/05'

records1=read_csv(os.path.join(dir1,"pointcloud_locations_20m_10overlap.csv"))
records2=read_csv(os.path.join(dir2,"pointcloud_locations_20m_10overlap.csv"))

pcl1=read_pcl(dir1,records1[5])
pcl2=read_pcl(dir2,records2[8])
